...
async function updateFrontmatter(
	...
	if (parsed.kind === "invalid") {
		...
		const msg = `Apple Health Sync: Frontmatter in "${file.path}" ist kaputt (${parsed.reason}). Ein Backup wurde unter _apple-health-sync/backups/ abgelegt. Bitte manuell reparieren.`;
		console.warn(msg);
...
/**
 * Writes a timestamped backup of the file content into _apple-health-sync/backups/.
 * Best-effort — failures are logged but not propagated.
 */
async function backupFile(app: App, file: TFile, content: string, reason: string): Promise<void> {
	try {
		const backupDir = "_apple-health-sync/backups";
		...
		const header = `<!-- Apple Health Sync Backup\nReason: ${reason}\nOriginal: ${file.path}\nTimestamp: ${ts}\n-->\n`;
		await app.vault.create(backupPath, header + content);
		console.warn("Apple Health Sync: backup written to", backupPath);
	} catch (e) {
		console.error("Apple Health Sync: backup failed for", file.path, e);
	}
}
